import 'package:birthflow_movil/src/core/chart/generators/curve_creator.dart';
  ...
  List<CurveData> _generate(
    ...
    if (cervicalList.isNotEmpty &&
      ...
      if (cervicalList.first.value >= 4.5) {
        return CurveCreator(realCurve.first, defaultValue).alertCurve();
      } else {
        if (realCurve.length >= 2) {
          ...
          for (int index = 1; index <= realCurve.length - 1; index++) {
            if (realCurve[index - 1].cervicalDilation <= 4.5 &&
              ...
              final double y2 = realCurve[index].cervicalDilation;
          ...
          return CurveCreator(data!, defaultValue).alertCurve();
        } else {
          return [];
        }
      }
